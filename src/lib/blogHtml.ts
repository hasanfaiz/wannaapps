import type { PortableTextBlock, PortableTextSpan, SanityBlogPost } from './sanityRest';
...
export function portableTextToHtml(blocks: PortableTextBlock[] = []) {
  ...
  for (const block of blocks) {
    ...
    if (block.listItem) {
      const current = block.listItem;
      if (listType !== current) {
        closeList();
        listType = current;
        html.push(current === 'number' ? '<ol>' : '<ul>');
      }
...
<body>
  <header class="site-header">
    <div class="container nav">
      ...
      <div class="nav-cta">
        <a class="phone" href="tel:+919884732100">Call +91 9884732100</a>
        <a class="btn" href="/contact-us/">Free Consultation</a>
      </div>
    </div>
  </header>
  ${body}
...
function postImage(post: SanityBlogPost) {
  return post.featuredImage?.url || post.featuredImageUrl;
}
...
export function blogListSchema(posts: SanityBlogPost[]) {
  ...
  return `<script type="application/ld+json">${JSON.stringify({
    ...
    '@type': 'Blog',
    name: 'WannaApps Digital Marketing Blog',
    url: `${siteUrl}/digital-marketing-blog/`,
    blogPost: posts.slice(0, 10).map((post) => ({
      '@type': 'BlogPosting',
      headline: post.title,
      url: `${siteUrl}/digital-marketing-blog/${post.slug}/`,
...
export function articleSchema(post: SanityBlogPost) {
  ...
  return `<script type="application/ld+json">${JSON.stringify({
    ...
    author: { '@type': 'Organization', name: 'WannaApps' },
    publisher: {
      '@type': 'Organization',
      name: 'WannaApps',
      logo: { '@type': 'ImageObject', url: `${siteUrl}/assets/wannaapps-logo.png` }
    },
